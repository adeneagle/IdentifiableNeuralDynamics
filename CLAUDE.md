...
| Numerical falsification | `experiments/` | exp01–exp10 pass, all JSONs current. **`exp11` and `exp12` intentionally report FAILING checks** — they encode predictions the *dynamics-only* regime refuted. Per §8 those are committed, not tuned away, so **neither is registered in `run_all.py`**, which covers exp01–exp10 only. Both are also **superseded in part by §3.12** and carry a banner saying so; read `exp13` instead for anything behavioural. Read their JSONs directly. **`exp14` is likewise unregistered, on cost (~25 fits) not outcome** — its parts 1–2 *are* the metric validation, run in seconds, and every claim in them is asserted in `tests/`, so the regression gate for the new machinery is the test suite. **`exp15` / `exp15b` / `exp16` are also unregistered, on cost** — `exp15` is ~70 min (150 fits) and `exp16` ~47 min (96 fits); `exp15b` is seconds and could be registered but needs the NWB. **All three need `data/*.nwb`, which is gitignored and re-fetched by `idyn.nlb.fetch()`**, so `run_all.py` must stay runnable without a download. Their JSONs carry every fingerprint, so scoring can be re-derived offline (`exp15_rescore.py`) without refitting. **`exp17` is unregistered on cost too** — ~63 min, 48 fits — but unlike the others it needs no download, and it **checkpoints its JSON after every arm**, so an interrupted run keeps what it had. Its companion `exp17_calibrate.py` is a settings calibration, not a result; its own JSON was never written (the run was stopped) and the rows it produced are recorded in that file's docstring |
...
| **REAL DATA — MC_Maze (tasks 39/40)** | `theory/identifiability.md` §10, `src/idyn/nlb.py`, `exp15`, `exp15b` | **NEW (2026-08-14). The repo is no longer 100% synthetic.** NLB MC_Maze (DANDI 000128, 1721 trials, 182 units, 108 conditions), movement-aligned, condition-averaged PSTHs — a condition *is* an initial condition, so this matches §1.1's deterministic-flow target rather than needing §3.8's distributional equivalence. **No `nlb_tools`**: everything is in the NWB. **(a) Task 39 — the modular constraint is free.** Held-out-*neuron* co-smoothing over unconstrained/triangular/modular: $0.2979/0.2981/0.2983$, indistinguishable, most-constrained marginally ahead. Metric is not dead — it moves $0.193\to0.421$ over $d=2\to10$, $100\times$ the restart spread. **(b) Task 40 — the rotation number is identifiable, the Lyapunov spectrum is not.** 3 splits × 6 restarts, disjoint 79/79 rate-stratified halves, against a **per-neuron circular-shift null** (preserves every single-neuron statistic, destroys only the shared latent): rotation $0.00208$ vs $0.02173$ ($10.4\times$), relative to module separation $0.112$ vs $36.9$ ($330\times$), lattice margin $0.00151$ vs $0.01553$ and beating the random-vector null $0.00228$ — but global spectrum $0.02645$ vs $0.02536$ and per-module $0.0288$ vs $0.0271$, i.e. **no signal at all**. §3.13(b) in a new form: 35 bins at $\lvert\lambda\rvert\approx0.99$ contract only $30\%$, so there is no contraction to measure, while phase advances throughout. **(c) `duplicate_modules` separates data from null outright** — $18/18$ control fits flagged vs $1/36$ treatment, with no ground truth. **(d) The flow is $\ge99\%$ linear, and this is about the DATA not the preprocessing** (`exp15b`): nonlinear share $0.09$–$0.53\%$ across every smoothing, window, dimension and dataset; unsmoothed $R^2$ falls to $0.838$ but quadratic recovers only $0.38$ of the missing $16$ points, so that gap is PSTH noise. **Both escapes are closed** — single trials (no averaging) give $0.15$–$0.23\%$, indistinguishable from the averaged $0.21\%$; and **MC_RTT**, a continuous unstructured task, is *less* nonlinear at $0.04$–$0.12\%$. At $d=4$ the fitted map's eigenvalues are complex-conjugate pairs, so **modularity is generic, not restrictive** — this validates **Theorem A**, not Theorem B. **The nonlinear theory cannot be tested on these benchmarks because the phenomenon is not in them**; the next real-data step is a different *regime*, not a better estimator. **(e) (F3) holds in 2 of 36 fits**, so the best empirical result again sits outside the best theorem (`exp14`: 0/24). **(f) My scoring had a vacuous-check bug**: the circshift control screens out completely, its screened arm was empty, and `NaN`$\to+\infty$ made 3 checks pass against nothing — $6/6$ became **$7/9$** once `NaN` fails and arms are compared like-for-like. Re-derived offline from the dumped fingerprints, no refit |
| **exp16 — the Route C instrument, calibrated** | `theory/identifiability.md` §11, `experiments/exp16_route_c_calibration.py` | **NEW (2026-08-15). Runs the task-40 protocol where the answer is CERTAIN, in both directions.** Two stages, deliberately separated: **analytic** (exact systems, no fitting — any failure is the *metric's*) and **fitted** (any extra failure is the *estimator's*). **(a) The metric is correctly calibrated, 6/6 exact**: blind to the §7.1 radial shear ($10^{-10}$), catches the §3.1 regrouping ($0.2231$), (F3) flips $+0.2231\to-0.2231$, torus rotation moves $0.2069$ with lattice margin exactly $0$, (F3) rejects two neutral cycles at $-0.9163$. **(b) THE HEADLINE, and it is negative: arm C is a false positive.** Two limit cycles are provably identified only up to $GL(2,\mathbb{Z})$, yet the two independently fitted halves agree to $0.0004$ under *both* decoders — **both fits land on the same lattice representative**. So **cross-split agreement is necessary but NOT sufficient**; the §10.3 protocol measures *estimator reproducibility across neuron samples*, not identifiability. This confirms task 40's own retraction experimentally, and §10.4 is weakened accordingly. **(c) Gauge-blindness of the metric $\ne$ gauge-robustness of the estimator**: arm D (arm A + shear) is blind at $10^{-10}$ analytically and recovers fine under a linear decoder, but fails under the MLP ($\rho$ err $0.0413$ vs null $0.0681$) with fitted (F3) collapsing $+0.488\to-0.038$. **(d) §10.3's real-data split reproduces under KNOWN ground truth**: arm A + MLP recovers rotation ($0.0030$ vs null $0.0550$) and not the spectrum ($0.1687$ vs $0.1897$) with (F3) *holding* at $+0.134$ — so "rotation yes, spectrum no" is an estimator property, not a fact about MC_Maze. **(e) (F3) is a one-way indicator**: $>0$ identified in $3/3$; $\le0$ identified in $2/5$, both exceptions being arm C. Positive (F3) is usable; non-positive is not a reliable negative. **(f) Verdict-rule caveat of the §3.9 family**: arm C scores as recovering the *spectrum* only because its two modules have identical spectra, so agreement there is free — a comparison that cannot fail. **Next was adversarial initialisation (§11.5); it is DONE — see the exp17 row below, which fixes (b).** |
| **exp17 — adversarial initialisation closes task 41** | `theory/identifiability.md` §12, `experiments/exp17_adversarial_init.py`, `train.warm_start_to_latents` | **NEW (2026-08-16). The exp16 false positive is fixed, and the verdict is now right in all four arms (18/19 checks).** Warm-start the two halves at *deliberately different* representatives, then train normally: if the data pins the representation the fits come back, if not they stay. **(a) The repair, on exp16's own metric — same data, same fits, only the initialisation differs.** Cross-split agreement, matched init $\to$ adversarial init: arm C (two limit cycles, *provably* non-identifiable) $0.00018\to\mathbf{0.1271}$, i.e. the false positive becomes a correct rejection; §3.1 regrouping $0.0211\to0.1165$; and the two arms that *should* agree still do (A $0.0049\to0.0207$, escape control $0.0020\to0.0033$). **(b) The scored readout is fit-to-*target*, not fit-to-fit** — reusing fit-to-fit would inherit the defect being repaired — and it is restricted to the modules the construction says separate R1 from R2. That restriction is forced: `invariant_agreement` reports a **max** over modules, and the donor's rotation number is never recovered ($0.0039$ against a true $0.1751$) while the module the map moves comes back to $3\times10^{-4}$, so an unrestricted comparison decides arm A on an invariant nobody measured. `AgreementReport` now carries `per_module_spectrum`/`per_module_rotation`. **(c) The escape control is what makes 'STAYED' mean anything** — arm E's $h$ is expressible but provably not a modular conjugacy ($HFH^{-1}$ is $0.4305$ off-block), and the fit abandons it to $0.0001$, so inertia is excluded. **(d) My pre-registered prediction was WRONG and the check is committed failing** (the 1 of 19): arm A's alternative is reachable in latent *values* to $6\%$, not $>25\%$ as predicted — but the warm-started *dynamics* still read R1. So (F1) does not make the alternative unrepresentable pointwise, it makes it unrepresentable **as a conjugacy**. Checking the fingerprint and not just the encoder residual is what showed this. **(e) Neither (F3) nor $\min\lvert z_i\rvert$ is a universal predictor.** For the lattice family (arms A/C, same $h$) §11.6 is confirmed exactly — $\min\lvert z_{donor}\rvert$ separates them by $10^8$ and **(F3) orders them backwards** ($+0.5146$ where the alternative dies, $-0.1453$ where it survives). But **arm B has (F3) $=+0.10$ *and* a donor radius of $10^{-5}$ and survives anyway**, because its $h$ is a permutation with $\lVert Dh\rVert=1$. So $\min\lvert z_i\rvert$ is the diagnostic *for the lattice family*, and **(F3) $>0$ does not pin the representative** — Theorem F buys the filtration, not the representative. **(f) Honest limits:** arm A's negative is weaker than arm C's positive (the warm start never put it *at* R2, so for that arm the test partly did not apply); arm B resolves its own question by only $3.1\times$; 4 restarts (all unanimous 4/4 or 0/4); and **it has not been run on MC_Maze**, so §10.3 stays 'estimator reproducibility'. §12.6 says how to run it there — the lattice image of the *fitted* latents is constructible without knowing the true $h$ |
| **exp18 — Route B does NOT rescue the lattice ambiguity, and the reason is a symmetry** | `theory/identifiability.md` §13, `experiments/exp18_behaviour_vs_lattice.py` + `exp18_mechanism.py`, `systems.rotational_behavioural_escape` | **NEW (2026-08-17). Asks whether behaviour kills the $GL(2,\mathbb{Z})$ regrouping that Lemma C/Theorem F provably cannot. Answer: no.** 9/12 checks; the analytic pre-flight is 3/3 and decisive, the pre-registered headline is **refuted and committed failing**. **(a) Lemma D's own (D3) is unavailable in the oscillatory regime**: variance modulation is radial and a limit cycle attracts every radius to $\rho$, so the conditioning is erased — $u$-dep $0.799\to0.034$, mean-radius gap $6.0\times10^{-1}\to1.8\times10^{-12}$. So where Route C has no hypothesis, Lemma D has none either, and what is tested is the Route B *mechanism*. **(b) The escape §4.5 excluded becomes a genuine conjugacy.** Donor-first, the regrouping is $h_B=z_B z_A/\lvert z_A\rvert$ — a **rotation** of the invariant block, i.e. `nonadditive_behavioural_escape`, which was excluded because $\theta\circ f_A-\theta$ must be constant and a contracting *fixed point* forces it to $0$. On a **cycle** that increment is exactly $\omega_A$: defect $7.5\times10^{-16}$. **(c) Behaviour is blind to a coupling that acts by a symmetry of $p_B$** — rotationally symmetric $p_B$ gives $p(h_B\mid u)=p_B$ for every $u$; detector $0.016$ against its own $0.044$ floor (floor slope $-0.44$, checked). **General form: the kill needs $p_B$ to have trivial symmetry under the group the coupling takes values in — which is *why* Lemma D's additivity works, since no law is translation-invariant**, and why the escape needs $d_B\ge2$. **(d) Breaking the symmetry works in the DATA ($0.016\to0.956$ monotone in $\kappa_B$) and does NOT survive learning.** The $2\times2$ returns SURVIVED in all four cells; in the asymmetric behaviour-on cell the penalty pulled the block $0.961\to0.310$ and moved the fit off R2 ($0.0001\to0.0314$) but *away* from R1 ($0.0795\to0.1101$, past R2 — the separation is only $0.0796$) at $30\times$ fit-quality cost, while the matched arm sits at R1 with **both** lower penalty and better fit. **(e) Mechanism, measured: the encoder restores the symmetry.** Fitted invariant-block circular concentration — matched $0.809$ (data R1: $0.854$, flat across $u$) vs adversarial $\mathbf{0.270}$, **below the data's own R2 value $0.392$**. The encoder smears the phase *further* than the regrouping did. Unanimous over 3 restarts ($0.270/0.264/0.211$ vs $0.809/0.818/0.807$, $3.1\times$, no overlap). **And it holds *within* the treatment arm**, which is the stronger form: `exp18_mechanism` reuses `exp18`'s seeds, and per restart concentration $0.2703/0.2640/0.2106$ tracks $u$-dep $0.4359/0.4313/0.1886$ and distance-to-R2 $0.0495/0.0496/0.0130$ — **monotone in all three, so the restart that hides best scores best.** That is the signature of an *evaded* constraint, not an imposed one. Third instance of §3.12: a structural constraint paid in gauge. **(f) Vacuous check kept and marked**: `duplicate_modules` flags 4/4 in *every* cell including the $w=0$ controls, because two limit cycles have identical spectra by construction — the `exp15` trap, so check 9 cannot pass on this system. **Unregistered in `run_all.py` on cost (~2.8 h, 32 fits); checkpoints per restart** |
| **exp19 — the Hsu open-field data HAS the nonlinearity, in M56, and my first instrument could not see it** | `src/idyn/hsu.py`, `experiments/exp19_hsu_nonlinearity.py`, `tests/test_hsu.py` | **NEW (2026-08-23). Answers task 42 positively for one area, and corrects the instrument `exp15b` used.** Second real dataset: 4.5 h continuous, ~450 units over VS/DS/M56/M23, a 16-state ethogram (28k bouts), 36 DLC kinematic features. **(a) THE MEASUREMENT WAS ON THE WRONG OBJECT.** `exp15b.flow_linearity` scores curvature of the *flow*, inside a space PCA already chose. What gates Theorem B is curvature of the **decoder** — §3.5: a linear $g$ forces $h\in GL(d)$ before dynamics enter, and the §3.7 counterexample has **linear modules** with a nonlinear $h$. A flow statistic is structurally blind to it: if the manifold is curved, PCA returns a flattened shadow whose internal dynamics can be near-linear. The right test is the one a practitioner reaches for anyway — nonlinear autoencoder vs PCA at matched bottleneck, held-out. **(b) The raw gain is large and needs three controls, each removing a different artefact.** Raw $+0.13$–$+0.24$; against a matched mean/covariance Gaussian surrogate the same net is *worse* than PCA ($-0.017$ to $-0.061$), so the overfitting budget is calibrated and there is no free advantage. A Gaussian-**copula** surrogate (exact marginals) is reported but **biased up** — its rows are i.i.d. while the data is autocorrelated at the smoothing width, so it has several times the effective sample size; do not read it as decisive. **(c) THE DECISIVE CONTROL is coordinate-wise, and it is not cosmetic.** `sqrt` is applied per neuron, so it alone would bend a flat subspace into a curved manifold; and if $g=\phi\circ W$ with $\phi$ per-coordinate then two models sharing $\phi$ give $h=\tilde W^{+}W$, **linear** — §3.5 again, after a known change of variable. Rank-Gaussianising **each unit separately** (fitted on train only, so it cannot leak) removes every monotone per-neuron warp. Result: **only M56 survives, $+0.111$ at $k{=}4$ and $+0.124$ at $k{=}8$**; VS goes *negative* ($-0.030/-0.068$), DS collapses ($+0.051/+0.020$), M23 is inconsistent ($-0.025/+0.073$). So VS/DS/M23 sit in **Theorem A after a known change of variables** — a clean negative, not a failure to measure. **(d) Two unrelated statistics select the same area.** M56 is independently the only area whose *flow* curvature beats the per-neuron circular-shift null ($0.32$–$1.13\%$ against $-0.09$–$0.07\%$, i.e. $4$–$34\times$; the others sit at $1$–$2\times$). **(e) Both numbers are LOWER bounds** — the AE gain still climbs with budget ($0.200/0.225/0.256$ at $2$k/$4$k/$12$k steps, M23 $k{=}6$). **(f) Honest limits:** striatal units fire ~$10\times$ slower (median $0.3$ vs $3.1$ Hz), so their negatives may be SNR-limited rather than true flatness; and M56 has only **~27 units** above threshold in the best of 9 sessions, below the $\approx32$/side §3.13(e) needs for the disjoint-split agreement protocol — which bounds **task 40** here but **not task 41**, since `exp17`'s adversarial initialisation needs no neuron split. **(g) RETRACTED in passing:** an earlier reading of this session (real eigenvalues, (F3) above null, 'Route C looks available') was computed in **PCA space**. If M56's manifold is genuinely curved that is the wrong chart, so treat it as withdrawn for M56 pending a redo in a nonlinear latent space. Unregistered in `run_all.py`: needs `IDYN_HSU_ROOT` and runs ~21 min; checkpoints per area |
| **exp20 — Route B's kill is about COMPACTNESS, not additivity; and the detector is blinder than the criterion** | `theory/identifiability.md` §14, `experiments/exp20_stabiliser_dichotomy.py`, `tests/test_behavior.py` | **NEW (2026-08-24). Stage 1 of the Route B program: exact distributions, no fitting, 15/15.** Completes §13.3's slogan into **Proposition S**: for $h_B=T(z_A)\cdot z_B$ with $T$ valued in a group $G$ acting affinely and $p_B$ of finite nonsingular second moment, (D4) holds with $M_{BA}\neq0$ **iff** $T$ is valued in one coset of $\operatorname{Stab}_G(p_B)$ — and $\operatorname{Stab}_G(p_B)\subseteq\Sigma^{1/2}O(d_B)\Sigma^{-1/2}$, **compact**. One line: $A\Sigma A^\top=\Sigma$ and $A\mu+b=\mu$. **(a) Lemma D generalises well past additive $h_B$.** At *matched displacement* ($0.300$, bisected per arm, so "invisible" can never be "weak") translations $10.4\times$ floor, scalings $20.5\times$, shears $14.7\times$ — every non-compact group is killed. Additivity was never load-bearing; **non-compactness was**, and "no law is translation-invariant" is its $d_B$-dimensional shadow. **(b) The blind group is *conjugate* to $O(d_B)$, not equal to it** — against anisotropic $p_B$ a plain rotation is caught ($12.9\times$) while $\Sigma^{1/2}R\Sigma^{-1/2}$ is not ($0.95\times$), a $13.5\times$ split. **So making $p_B$ anisotropic relocates the escape rather than removing it.** **(c) THE HEADLINE, pre-registered as a negative and it landed: the detector is blinder than the criterion.** A centred, exactly-whitened, asymmetric $p_B$ (trivial $\operatorname{Stab}_{O(2)}$, 4th harmonic $0.165$) under a rotation coupling — (D4) fails **grossly**, 3rd circular harmonic gap $0.2134$ vs $0.0030$ uncoupled ($71\times$) — while `block_u_dependence` reads $0.75\times$ **its own floor**. Separating those two was the point; without the harmonic test "at floor" is unattributable. **Consequence, and it is a real tension: §3.12's whitening fix and §13.3's rotational blindness are the SAME FACT.** Whitening makes second moments isotropic by construction, so a $GL(d_B)$-invariant second-moment detector is blind to $O(d_B)$-valued couplings for **every** $p_B$ — the gauge-invariance we require is exactly what forfeits the orthogonal direction. **(d) What survives is a $u$-dependent MEAN DIRECTION, with a measured threshold.** Sweeping $\lVert\mathbb{E}z_B\rVert$: concentration $0.001/0.155/0.303/0.557/0.844$ gives $0.77/0.90/\mathbf{11.9}/25.3/27.4\times$ floor — monotone, and a **threshold between $0.155$ and $0.303$**. **That bracket is where `exp18`'s fitted adversarial block sits ($0.270$)**, against its matched arm at $0.809$ — so §13.4's "the encoder restores the symmetry" is quantitatively predicted by an experiment containing no fitting: **the encoder is centring the block**, and only has to get concentration under $\approx0.3$. Design rule for stage 2: penalise *whitened* circular concentration, which is $GL(d_B)$-invariant ($O(d_B)$ fixes it, whitening kills the rest) so §3.12 cannot recur. **Honest limits:** this measures **(D4) and its detector**, not whether $T$ is a genuine modular conjugacy (§13.2 — the rotational escape is excluded at a fixed point and real on a cycle); $p_B$ needs finite nonsingular second moment; $d_B=2$ throughout; and nothing here says the concentration condition survives learning. Floor measured, not assumed (log-log slope $-0.509$ in $n$). Seconds to run, but unregistered in `run_all.py` pending the stage-2 companion |
| **exp21 — Route D: independence of the module marginals, and it rejects the two counterexamples that block Theorems B and F** | `theory/identifiability.md` §15, `experiments/exp21_resultant_canonicalises.py`, `tests/test_behavior.py`, `tests/test_counterexamples.py` | **NEW (2026-08-24). 25/26; the one failure is committed and is the informative one.** Stage 2 of the Route B program, and it does not go where stage 1 pointed. **(a) The planned fix is DEAD, caught before running it.** Imposing whitened concentration as an objective is fakeable: free maximisation reaches $0.9996$ by giving the block's radius a heavy tail (spread $524\times$), since whitening pins the second moment and leaves the radius free — §3.12 a *fourth* time. So the statistic must be a **comparison across fits**, not a loss term. **(b) Proposition C, proved and exact:** for $h_B=T(z_A)z_B$ with $T$ orthogonal-valued and independent of $z_B$, $\mathbb{E}[h_B/\lVert h_B\rVert]=\mathbb{E}[T]\,\mathbb{E}[z_B/\lVert z_B\rVert]$, so the directional resultant **strictly decreases** unless $T$ is a.s. constant (in $d_B{=}2$, the circular convolution theorem — resultants multiply, verified to $1.8\times10^{-3}$). **So Proposition S's compact residue moves a scalar functional monotonically, and is canonicalisable with no behavioural variable at all.** **(c) The statistic had to change and the near-miss is instructive:** `exp18` used raw $\lvert\mathbb{E}e^{i\phi}\rvert$, not shear-invariant, hence a gauge quantity. Whitening by the **uncentred** second moment is exactly $GL(d_b)$-invariant. Whitening by the *covariance* — centring, as every other detector here does — scores a concentrated block $0.0177$ and a phase-randomised one $0.0196$, i.e. **blind**, because centring removes the mean direction that §14.4 says is the whole signal. **(d) THE HEADLINE: independence is the real criterion, and it is a different route.** In the true representation the modules' phases are independent; a lattice image makes them dependent. Against the repo's counterexamples (dCor baseline $0.065$): §4.3 triangular $0.531$ ($8.2\times$) **rejected** — the object that makes block-diagonality *false* under (B1)–(B4) — §7 lattice $0.623$ ($9.6\times$) **rejected**, §3.1 regrouping $0.077$ ($1.2\times$) **correctly blind**, since that is (B2)'s job. Needs no non-resonance (identically false for oscillators, Prop. N), no (F3), no auxiliary variable. Margins over the $GL(2,\mathbb{Z})$ orbit are $98\times$/$31\times$ against the resultant's $4$–$75\%$. **(e) Two escapes bound it, and one is free.** Hyvärinen–Pajunen is real and lives inside our setting — with *uniform* phase laws the lattice preserves independence exactly ($0.8\times$ baseline) — so **Route D is a conjecture**: independence *plus modular conjugacy* is what must be rigid. The Gaussian degeneracy is also real but **excluded by (A2)**, since a rotation is a modular conjugacy only at equal eigenvalues. **(f) The proviso is the opposite of the obvious one:** $\phi_i+\phi_j\perp\phi_j$ iff **$\phi_i$** is uniform, not $\phi_j$ — adding an independent uniform makes the *marginal* uniform but not the pair independent. So a single uniform module already opens an escape, and Prop. R′ needs *no* module uniform. Measured at $\kappa=(4,0)$: $(1,0;1,1)$ ties, $(1,1;0,1)$ is rejected at $0.86$. **(g) Under learning, 3/3 for the resultant and 2/3 for independence — and the failure is the theory's own hole realised by gradient descent.** Refitting `exp18`'s arms: matched resultant $0.7154/0.7130/0.7116$ vs adversarial $0.2833/0.2828/0.2055$ (data R1 $0.7449$, R2 $0.3912$), radial tails $1.5$–$1.7$ so no degeneracy — and `exp18`'s conclusion survives the gauge-invariant statistic. But restart 2's adversarial fit is as *independent* as the matched one ($0.272$ vs $0.278$): it is also the arm with the lowest resultant and lowest $u$-dependence, i.e. **the encoder flattened the phase toward uniform, which is exactly where the lattice becomes independence-preserving.** So the two criteria are complementary and both must be reported. **(h) Sensitivity, with the boundary measured:** the *relative* resultant deficit is $1-R_\theta$ by Prop. C, hence scale-free in $R_{z_B}$ ($15.6/16.2/15.2/13.3/10.6\%$) while behaviour's signal-to-floor scales with it ($4.2\to58\times$) — so the resultant dominates in the weakly-concentrated band, and **both die at $R_{z_B}=0$**. Analytic parts are seconds; part 4 is 6 fits (~35 min) and checkpoints per restart. Unregistered in `run_all.py` on the fitting cost |
| **exp22 — Route D survives learning, and the blind control stays blind** | `theory/identifiability.md` §15.14, `experiments/exp22_independence_under_learning.py`, `metrics.procrustes_block_distance`, `models._independence_penalty` | **NEW (2026-08-24). Task 48. 5/6; the one failure is in the fit-quality budget, not in any structural prediction.** Reuses `exp18`'s system, seeds and settings so the only new thing is `w_independence`; distances are the §3.17 Procrustes distance, so the within-module gauge is quotiented out. **(a) The $w=0$ row is the within-experiment A/B and it carries the result.** Unpenalised, an adversarially-initialised fit **stays** at R2 ($d(R_2)=0.0274$ against $d(R_1)=0.9709$, fitted dCor $0.7966$) — `exp18` reproduced, the lattice representative is stable under learning. One term flips it. **(b) It arrives *at* R1 rather than drifting off R2.** Adversarial lands at $0.324$–$0.350$; the arm *initialised* at R1 lands at $0.275$–$0.443$ — the same distribution, so $\approx0.33$ is the estimator's floor, not a shortfall. Pre-registered contrast with `exp18`, whose behavioural penalty moved off R2 but *past* R1 (§13.4). **(c) THE CONTROL, unanimous: the symmetric cell does NOT move, 3/3.** With $p_B$ rotationally symmetric the lattice image is genuinely independence-preserving in the data (dCor $0.0687$ at R2 versus $0.0770$ at R1 — *more* independent at R2), so Theorem D′ says the term must be blind there, and it is, at the same weight and the same budget. **A penalty that merely degraded the fit into noise would have moved this cell too.** This is the check §3.12, §3.15 and §13.4 would each have failed. **(d) FAILING CHECK, committed:** matched-arm fit quality $2.152\times10^{-3}$ against the $w=0$ calibration's $3.255\times10^{-4}$, a factor $6.6$ where the pre-registered bar was $5$ — and the reference ran 800 steps against the arm's 3000, so the true cost is *at least* that. **A fresh instance of §3.11 one level over: a step count does not survive a change of observation model, and here a *weight* did not survive a change of *budget*** ($4.8\times$ at 800 steps, $6.6\times$ at 3000). Calibrate at the budget you will use, or report both. **(e) One pass is uninformative and is marked so.** `pred 4` cleared by $10^{-4}$ and compares dCor across two *different* data-generating systems, in one of which R2 is independent by construction — the `exp15`/`exp18` vacuous-check pattern. The within-system version is unavailable because no asymmetric fit stayed at R2. **Honest limits:** $K=2$, $d_B=2$, 3 restarts, one system, and the weight's fit cost has not been shown to be small at a matched budget. ~101 min, 15 fits; checkpoints per fit; unregistered in `run_all.py` on cost |
| **(F1), not (F3), is what protects a contracting module from the lattice** | `theory/identifiability.md` §11.6–11.7, `tests/test_counterexamples.py` | **NEW (2026-08-15), and it refines §7/task 23.** The $GL(K,\mathbb{Z})$ ambiguity is **not** special to limit cycles. Two contracting spirals ($s=0.92, 0.55$) with **(F3) holding at $+0.50$** still admit the exact regrouping $h(z_1,z_2)=(z_1z_2/\lvert z_2\rvert, z_2)$ carrying $\omega_1\mapsto\omega_1+\omega_2$ — residual $6.7\times10^{-16}$ at donor radii $1$, $10^{-2}$, $10^{-6}$ alike. **So spectral separation does not protect the rotation numbers.** What does is **(F1)**: $\lVert Dh\rVert\sim1/\lvert z_2\rvert$, and the donor spiral decays to $1.6\times10^{-8}$ over the trial ($\sup\lVert Dh\rVert\approx6\times10^7$, (F1) fails) where a limit-cycle donor stays at $0.80$ ($\approx1.2$, (F1) holds). **The lattice ambiguity bites exactly when the donor module does not decay** — §3.8's support caveat doing real work. **Checkable and worth reporting beside any rotation claim:** $\min\lvert z_i\rvert$ over the visited region, a *different* diagnostic from `filtration_gap`. **Also corrects §11.3(a)'s diagnosis:** exp16's fitted model has a **linear encoder**, so $\hat z=L\,g(z)$ is linear in $z$ under a linear generating decoder and the nonlinear alternative is **outside the fitted class** — the protocol could not have detected it whatever the optimiser did. Hence the §11.5 adversarial test **requires `encoder="mlp"`**; warm-starting a linear encoder at a nonlinear representative measures the projection, not the data. **Trap recorded: `TwistBlock.beta` defaults to 0.6**, so an unqualified `TwistBlock(s, omega)` is *sheared*; that default silently broke my first version of this check |
| Test suite | `tests/` | **431 tests, 430 passing + 1 skipped** (was 393 before the Route D work: exp21, Theorem D, the independence term, section 3.17, then exp22 plus the Theorem D-prime linearisation boundary and Lemma T). The skip is `tests/test_hsu.py::test_real_session_loads_and_is_self_consistent`, which needs `IDYN_HSU_ROOT`; every other property of `idyn.hsu` is pinned against a synthetic session built in-process, so the suite still passes on a fresh checkout with nothing downloaded. `test_naive_sigma_min_route_is_noise_on_a_limit_cycle` is fixed per §4.1's own prescription — it now asserts only the clause true on *both* numpy 2.4 and 2.5, namely that the naive rate misses the truth by more than half the spread. New datum from the rewrite: at `n_max=200` the naive rate is **+0.73 against a true −0.29**, i.e. it returns the wrong *sign* — a false **negative** on Lemma C, mirroring the known false positive |
...
### 3.15 The behavioural penalty pooled timesteps, which erases a *rotating* signal

**Seventh defect of the §3.9 family, and the second in the objective rather than
a readout** (§3.12 was the first). Found in `exp18`.

`models._behavioural_penalty` stacked every timestep into one sample set before
whitening and scoring. An oscillatory block's phase advances by $\omega$ each
step and wraps several times across a trial, so the **time-pooled** law is close
to rotationally uniform for every $u$ — even when the per-timestep laws differ
completely.

Measured on `exp18`'s data, the recipient block under the lattice regrouping:

| | per-timestep | time-pooled |
|---|---|---|
| recipient, true representation | $0.191$ | $0.0044$ |
| recipient, **under R2** | $\mathbf{0.979}$ | $\mathbf{0.0166}$ |
| donor (the $u$-varying block) | $0.991$ | $0.106$ |

A $59\times$ collapse that leaves R2 only $3.8\times$ above a genuinely invariant
block. **Consequence: no weight can impose the constraint** — the pooled penalty
scores $2.8\times10^{-4}$ at R2, the same order as the fit terms themselves, and
a sweep over $w \in \{1, 5, 20, 80\}$ left the fitted block at $u$-dependence
$1.01$ throughout while the penalty reported itself satisfied. Same shape as
§3.12: the objective was measuring something other than the hypothesis.

**Fix:** score each timestep and average, whitening per timestep too (a common
whitener reintroduces the pooled geometry). R2 then scores $1.080$ against R1's
$0.044$. `TrainConfig.behavior_per_time` defaults to `True`; `False` reproduces
the pooled form.

**`exp13` is untouched, and this was measured rather than assumed.** A *radial*
modulation does not rotate, so pooling only mixes decay stages: injecting
$z_B \mathrel{+}= 0.5\,z_A$ moves the pooled score $0.00085 \to 0.402$ ($473\times$)
against the per-time $0.00169 \to 1.275$ ($754\times$). Same order.

**The fix has a price, and it must be paid in batch size.** Scoring per timestep
leaves `batch`/$n_u$ points to estimate each conditional covariance, so the
penalty acquires a large positive sampling bias. On a **truly invariant** block:

| batch | 64 | 128 | 256 | 512 | 2048 |
|---|---|---|---|---|---|
| floor (invariant block) | $0.510$ | $0.312$ | $0.224$ | $0.063$ | $0.012$ |
| signal (at R2) | $1.107$ | $1.087$ | $0.992$ | $0.976$ | $1.000$ |
| ratio | $2.2\times$ | $3.5\times$ | $4.4\times$ | $15.4\times$ | $83.7\times$ |

At batch 64 half of what the optimiser minimises is noise, and it pays that down
by **distorting the block**: the matched arm drifts $0.032$ from R1 (40% of the
R1–R2 separation) and the block is driven to $u$-dependence $0.096$, *below* the
true block's own $0.211$. **Do:** report the penalty's floor at the batch
actually used, next to any claim that the constraint was imposed. **Do not**
carry a weight across the `behavior_per_time` flag any more than across
`behavior_whiten` — the absolute scales differ by ~$3\times$.

> Regression tests in `tests/test_behavior.py`: the pooled form's blindness to a
> rotating $u$-dependence, and the radial regime's insensitivity to the flag.

### 3.16 Never score a flexible model on a *random* holdout of an autocorrelated series

**Eighth defect of the §3.9 family, found while measuring the Hsu open-field
data (§14).** Same shape as the others — a plausible number from a misspecified
readout — but it is the first one in the *validation split* rather than in a
metric, a loss, or a hypothesis.

`flow_linearity`-style measurements compare a linear one-step map against a
quadratic one. Held out **at random over samples**, a test bin's immediate
neighbours sit in the training set; after smoothing, neighbouring bins are
nearly the same point, so the more flexible model memorises the local shape and
is rewarded for it. The bias is *not* symmetric between the two models — it
scales with capacity — so it inflates precisely the quantity being read.

**The tell, and it is worth remembering because it is unmistakable: the null
scored higher than the data.** On bout-averaged Hsu, per-neuron circular shift
gave a nonlinear share of $15.8\%$ against the data's $5.8\%$ (M56), and
$20.2\%$ against $5.3\%$ (M23). A surrogate with no cross-neuron structure
cannot be more nonlinear than the data; the ordering itself was the evidence.

**Fix:** hold out **contiguous blocks** whose length greatly exceeds the
smoothing kernel — whole segments for a continuous recording, whole time-blocks
within a condition for an averaged one. With the fix the same bout-averaged
comparison returns a *negative* gain at every setting, i.e. the quadratic model
genuinely does not generalise there, which is the honest answer.

**Consequence for `exp15b`, and it is mild but should be stated: that
measurement is in-sample.** `flow_linearity` fits and scores on the same array,
with no holdout at all. For MC_Maze's condition averages the resulting optimism
is small — $\approx10$ extra parameters against $3780$ samples at a linear
residual of $\approx1\%$, so of order $0.003\%$ against a reported $0.09$–$0.53\%$
— and it biases *towards* finding nonlinearity, so correcting it makes exp15b's
"the flow is $\ge99\%$ linear" conclusion **stronger, not weaker**. No retraction;
read the published numbers as upper bounds.

**Do:** block holdout, block length $\gg$ smoothing width, and report the null
at the same setting. **Do not** compare a quadratic gain against zero, or across
two arms whose effective sample sizes differ.

### 3.17 A distance to a reference representative must quotient the within-module gauge

**Ninth defect of the §3.9 family, found in `exp22` stage 0 before it cost a
54-minute run.** §3.10 caught a *metric* that measured the gauge and §3.12 an
*objective* that did; this is the same error in a **verdict** — the readout that
decides which representative a fit landed on.

`exp22` scores a fit by its distance to R1 and R2 after per-module whitening.
That is not enough: whitening removes scale and shear but leaves $O(d_i)$, and
§7 grants exactly that freedom. So a fit sitting **at** a representative up to a
within-module rotation scores a large distance and reads as a failure.

Measured, and it is genuinely ambiguous rather than merely noisy: with the
penalty on, raw $d(R_1)=0.83$ against $d(R_2)=1.03$. That is equally consistent
with *"moved off R2 to nowhere"* and with *"arrived at R1 in rotated
coordinates"* — the two hypotheses the experiment exists to separate.

**Fix:** align each module by orthogonal **Procrustes** before measuring. That
quotients precisely the residual freedom. Certified: identical latents score
$5.8\times10^{-17}$, a $40°$ within-module rotation $3.4\times10^{-16}$, a $5\times$
rescale $3.4\times10^{-11}$, and the true R1–R2 separation stays at $0.9709$.

**Do:** quotient the gauge in any fit-to-target distance, and check the
quotient is neither vacuous (the true separation must survive) nor incomplete
(a pure gauge change must score zero). **Do not** assume per-module whitening is
the whole gauge — it is $GL(d_i)$ modulo $O(d_i)$, and the $O(d_i)$ is the part
that bites a *distance*, though not a *distribution* statistic like §14's
resultant or §15's dCor, both of which are already $O(d_i)$-invariant.
...
2. `python -m pytest -q` — **expect 430 passing and 1 skipped**, on numpy 2.4 *or* 2.5; the
...
| **39** | **NEW: co-smoothing adequacy gate on real data** | **DONE 2026-08-14 (`exp15`, `identifiability.md` §10.2). Answer: the modular constraint is free.** Held-out-neuron co-smoothing on MC_Maze over the nested ladder gives $0.2979$ (unconstrained) / $0.2981$ (triangular) / $0.2983$ (modular) — indistinguishable over 5 restarts, with the *most* constrained marginally ahead, i.e. equal bias and lower variance. Caveat (a) of the original entry ("fits worse ≠ structure absent") does not bite, because the unconstrained model does **not** win; caveat (b) (optimisation artifact) does not either, since `fit_quality` is $0.6268/0.6273/0.6286$. **The metric is not degenerate** — it moves $0.193\to0.421$ across $d=2\to10$, $100\times$ the restart spread, which is the §3.11 control that makes a flat ladder readable. **But `exp15b` caps how much this can mean:** the latent flow is $\ge99\%$ linear and at $d=4$ block-diagonalisable over $\mathbb{R}$, so modularity is *generic* here — the constraint is not falsified, and it is also barely tested. `TriangularTransition` was built for this (it did not exist). Original entry: open — §6 "empirical program". Nested ladder unconstrained ⊃ triangular ⊃ block-diagonal, scored by held-out-**neuron** co-smoothing (fit ~80% of the population, refit a *fresh* decoder to the remaining ~20%). Target: Neural Latents Benchmark maze/RTT — standardised, and carries LFADS baselines. **This would be the first real data in the repo**; everything so far is `make_dataset`. Answers *is the structure there*, i.e. it settles diagonal-vs-triangular empirically instead of by §7's genericity argument. **It cannot answer identifiability** — the metric is constant on gauge orbits, by construction |
| **40** | **invariant agreement across disjoint neuron splits** | **RUN ON REAL DATA 2026-08-14 (`exp15`, §10.3). Answer: the rotation number is identifiable; the Lyapunov spectrum is not.** MC_Maze, 3 splits × 6 restarts, disjoint 79/79 rate-stratified halves, scored against a **per-neuron circular-shift null** that preserves every single-neuron statistic (smoothness, autocorrelation, marginals) and destroys only the cross-neuron alignment: rotation $0.00208$ vs $0.02173$ ($10.4\times$); relative to the modules' own separation $0.112$ vs $36.9$ ($330\times$); GL(2,$\mathbb{Z}$) margin $0.00151$ vs $0.01553$, also clearing the random-vector null $0.00228$. **But the spectra carry no signal** — global $0.02645$ vs $0.02536$, per-module $0.0288$ vs $0.0271$, i.e. the control is marginally *better*. Mechanism is §3.13(b) in a new form: 35 bins at $\lvert\lambda\rvert\approx0.99$ contract $30\%$ total, so there is no contraction to measure, while phase advances throughout — **a quantity is identifiable only if the data moves along it.** This *inverts* the expectation: the spectrum is the theoretically cleaner invariant (Tier 1, immune to the lattice) and is the empirically useless one. **Bonus, ground-truth-free: `duplicate_modules` flags 18/18 control fits vs 1/36 treatment**, separating data from null outright. **(F3) holds 2/36**, so this sits outside Theorem F exactly as `exp14` did (0/24). **Scoring bug caught and corrected**: the circshift control screens out completely, so its screened arm was empty and `NaN`$\to+\infty$ passed 3 checks against nothing; $6/6\to$**$7/9$** once `NaN` fails and arms match. Re-derived offline from dumped fingerprints, no refit — which is what §3.13 keeps them for. Prior synthetic entry: **METRIC BUILT AND VALIDATED (`exp14`); works under a linear observation map, open under a strong nonlinear one.** The test: fit independently on disjoint neuron subsets, then compare the *fits to each other* — identifiable dynamics give different coordinates and the same invariants. Varies the **data**, not the seed, so unlike restarts it excludes "artifact of this sample of neurons". Machinery: `metrics.dynamical_fingerprint` + `invariant_agreement`, plus `spectra.rotation_number` for the invariant spectra provably cannot see. **Validated on exact systems** — blind to a within-module gauge change *and* to the §3.7 triangular conjugacy, not blind to the §3.1 regrouping ($0.223$) or a frequency change; both directions are required and passing only one is easy. **Linear decoder: works** — 16/16 cross-split comparisons agree, rotation error median $2.5\times10^{-4}$ (max $5.1\times10^{-4}$), with signs *and* module order differing freely between fits, i.e. the gauge correctly quotiented out. **Negative control: works** — a frequency change is detected at $0.0638$ against a true separation of $0.06366$, 0/16 below threshold. **Transverse Lyapunov exponent does NOT agree** (median $0.052$, $208\times$ worse than $\rho$ on the same fits) — §3.13(b). **Nonlinear decoder (dose $0.574$, 8 neurons/side): fails, and not from undertraining** — $3\times$ the budget improves `fit_quality` $2.1\times$ and makes recovery *worse* ($0.080\to0.125$); it is **population size** up to $\approx32$ neurons/side, after which it plateaus. **What then remains is per-restart mode collapse** — 2 of 12 fits put both modules on one factor — and neither `coherence` ($r=-0.48$) nor `fit_quality` ($r=+0.24$, wrong sign) flags it. `DynamicalFingerprint.duplicate_modules` does, with no ground truth — screening on it takes the nonlinear arm's median $\rho$ error $0.0633 \to 0.0032$ ($20\times$) while leaving the negative control at $0.0636$, which is what makes it a fix rather than a filter. **Protocol for real data: many restarts, screen on duplicate invariants, report the *fraction* of agreeing splits plus the median, never a max.** `exp14` is **11/11** with that protocol. Full numbers in §3.13(e). **Still open: the residual 33% (a fit that misses a factor rather than duplicating one — undetectable without truth), and any real data at all.** **NEW 2026-08-12 — this validation is *outside* Theorem F, now quantified:** re-scoring the 24 saved fingerprints against the new (F3) gate gives `is_filtration` in **0 of 24**, median chain gap $-0.65$. Correct and by design (two limit cycles = rider 2), but it means the agreement is evidence that **(F3) is sufficient, not necessary**, and that the rotation number does work no theorem here licenses — i.e. support for the task-23 conjecture rather than for Theorem F. `identifiability.md` §6.5. **RETRACTED the same day — the task-23 conjecture is false** (§7 of `counterexamples.md`, task 23). Only the $GL(2,\mathbb{Z})$ orbit of the rotation vector is identified, so the per-module $\rho$ these fits agree on are **not** invariants of the data; both land on the same lattice basis for reasons of parameterisation, not because the observations pin it. **And the negative control's margin is 4× smaller than reported:** quotienting by $GL(2,\mathbb{Z})$ it sits $0.0159$ from an image of the true system, not $0.0637$. It still rejects, with a quarter of the headroom — use `spectra.rotation_lattice_margin`, which `invariant_agreement` now reports as a note (never scored: whether the ambiguity bites depends on shear, which a fingerprint does not carry) |

| **41** | **adversarial initialisation — turn agreement into evidence** | **DONE 2026-08-16 (`exp17`, §12). It works: verdict correct in all four arms, 18/19 checks.** The fix is stated below and it is what was built. **On exp16's own metric, matched init $\to$ adversarial init:** arm C $0.00018\to\mathbf{0.1271}$ (false positive becomes correct rejection), §3.1 regrouping $0.0211\to0.1165$, while the two arms that should agree still do. **The scored readout had to change too** — fit-to-*target* on the informative modules, since `invariant_agreement`'s max would decide arm A on the donor's rotation number, which is never recovered. **The escape control is load-bearing**: a map that is expressible but provably not a modular conjugacy is abandoned to $0.0001$, which is what excludes optimiser inertia. **Pre-registered prediction refuted and committed failing:** arm A's alternative is reachable in latent values to $6\%$, not $>25\%$ — but its warm-started *dynamics* still read R1, so (F1) makes the alternative unrepresentable **as a conjugacy** rather than pointwise. **New negative result:** (F3) $>0$ does NOT pin the representative (arm B has $+0.10$ and survives), and $\min\lvert z_i\rvert$ is the diagnostic *for the lattice family* only (arm B's donor decays to $10^{-5}$ and its permutation still has $\lVert Dh\rVert=1$). Original entry: `exp16` §11.3(a) showed the task-40 protocol returns **"identified" on a system where non-identifiability is proved** (arm C, two limit cycles, agreement $0.0004$). So cross-split agreement measures **estimator reproducibility**, not identifiability. **The fix:** start the two fits at deliberately *different* representatives — `regrouping_counterexample`'s `system_tilde` for §3.1, `torus_regrouping_counterexample`'s lattice image for §7 — warm-start on those latents, then train normally. **Requires `encoder="mlp"` (§11.7)**, and **report $\min\lvert z_i\rvert$ per module** (§11.6). **Successor is task 43: run it on MC_Maze**, which §10.3's conclusion still needs before it can be called identifiability |
| **43** | **NEW (ACTIVE, the front line): run the adversarial test on real data** | open. `exp17` closed task 41 on synthetic systems, so the *method* now returns the right answer where the answer is known — but §10.3's MC_Maze result is still **estimator reproducibility across neuron samples, not identifiability**, because the adversarial arm was never run there. It now can be, and the obstacle I expected does not bite: **the alternative representative does not require knowing the true $h$**. The fitted model supplies the modules, so the lattice image of the *fitted* latents ($z_1 \mapsto z_1 z_2/\lvert z_2\rvert$ applied to the fit's own output) is constructible directly, warm-started, and refitted. Report $\min\lvert z_i\rvert$ per module beside it, **not** `filtration_gap` (§12.5). **Expect the ambiguity to be live:** MC_Maze sits at $\lvert\lambda\rvert\approx0.99$ over 35 bins, i.e. nothing decays, which is exactly the regime §11.6 says the lattice bites. Budget ~1 h per condition at `exp17`'s settings; it checkpoints per arm |
| **44** | **NEW: Route B was asked to finish what Route C cannot, and it cannot either** | **ANSWERED NEGATIVELY 2026-08-17 (`exp18`, §13).** The motivation was sound — Lemma D′ needs no spectral hypothesis, so it applies exactly where Lemma C, Theorem F and (D1) are dead, and its own witness has identical spectra. Three findings, in order of how much they cost. **(a) Lemma D's (D3) is unavailable on a cycle** — radial modulation is erased by the attractor ($6.0\times10^{-1}\to1.8\times10^{-12}$), so only the *mechanism* can be tested, not the lemma. **(b) The regrouping acts by a rotation of the invariant block, and a rotationally symmetric $p_B$ is exactly invariant under it**, so $p(h_B\mid u)=p_B$ for every $u$. This is `nonadditive_behavioural_escape` — excluded at a fixed point by Step 1 — becoming a **genuine modular conjugacy** on a limit cycle, where $\theta\circ f_A-\theta=\omega_A\neq0$. **General form: the kill requires $p_B$ to have trivial symmetry under the group the coupling takes values in.** Additive $h_B$ works because no probability law is translation-invariant — so Lemma D's additivity was load-bearing, not a convenience. **(c) The obvious fix fails for a non-obvious reason.** Breaking $p_B$'s symmetry in the data restores detectability ($0.016\to0.956$), but the fitted encoder *flattens the phase back*: adversarial concentration $0.270$ against matched $0.809$ and the data's own R2 value $0.392$. So the condition must hold **in every representation the model can reach**, not just in the data. **Consequence for the route table: B∘C is not the fallback for the oscillatory regime.** What remains checkable on real data is the concentration diagnostic — report it beside any $u$-dependence, or an evaded constraint reads as an imposed one |
| **42** | **NEW: a regime with genuine nonlinearity** | **ANSWERED POSITIVELY 2026-08-23 for ONE area (`exp19`): M56 in the Hsu open-field data has decoder curvature that survives every control, $+0.121$ at matched bottleneck after per-unit Gaussianisation.** That is the precondition Theorem B needs and the NLB benchmarks lack. **The search framing below was right but the instrument was wrong**: `exp15b` measured curvature of the *flow* when §3.5 says what matters is curvature of the *decoder*, and a flow statistic cannot see decoder curvature because PCA flattens it first. VS/DS/M23 are a clean negative — their curvature is coordinate-wise, hence Theorem A after a known change of variables. Successor is task 45. Original entry: open, and it is a *search* problem rather than an estimator problem. `exp15b` closed every escape on the NLB benchmarks: the latent flow is $\ge99\%$ linear across smoothing, window, dimension, dataset, **single trials** ($0.15$–$0.23\%$, so averaging hides nothing) and **MC_RTT** ($0.04$–$0.12\%$, *less* nonlinear than the maze). So Theorem B is not merely untested there — **the phenomenon is absent**, and no better estimator changes that. Candidates: a longer horizon, an area with genuine multistability, a task that leaves the near-linear operating point, or accepting that for motor cortex in the movement epoch the proved linear theory is very nearly the whole story |
| **48** | **does Route D survive learning?** | **ANSWERED POSITIVELY 2026-08-24 (`exp22`, §15.14). 5/6, and the failure is in the fit-quality budget rather than in any structural prediction.** The three cells came back exactly as pre-registered: adversarial+asymmetric moves off R2 and lands **at** R1 ($0.324$–$0.350$, indistinguishable from the arm *initialised* there at $0.275$–$0.443$, so $\approx0.33$ is the estimator floor and there is no overshoot — the contrast with `exp18`, which went *past* R1); matched stays at R1 3/3; and **the symmetric control does not move, 3/3**, which is what excludes "the penalty just perturbs the fit". The $w=0$ row is the within-experiment A/B: unpenalised, the adversarial fit **stays** at R2 ($d(R_2)=0.0274$), so one term flips it. **Failing check, committed:** the penalty costs $6.6\times$ in fit quality against a $5\times$ bar — and it is a new instance of §3.11, a *weight* that did not survive a change of *budget* ($4.8\times$ at 800 calibration steps, $6.6\times$ at 3000). **Successor:** recalibrate $w$ at the full budget, and extend past $K=2$/$d_B=2$/3 restarts. Original entry: open, and **the objective term is built and validated** — `models._whitened_dcor` / `_independence_penalty`, `TrainConfig.w_independence`. Whitened dCor between modules: whitening quotients $GL(d_b)$ to $O(d_b)$ and dCor is orthogonal-invariant, so the term is **exactly $GL(d_b)$-invariant**, and unlike the §14 concentration term it is a *joint* statistic so it cannot be paid by reshaping one block's radial distribution (the §15.2 degeneracy). Validated analytically on `exp18`'s system: true representation $0.098$ vs lattice image $0.579$ ($5.9\times$), $GL$-invariant to $5\times10^{-3}$ under scale/shear/anisotropy, differentiable with finite gradients — **and blind exactly where Theorem D′ says it must be** ($0.081$ vs $0.082$, ratio $1.01$, when $p_1$ is rotationally symmetric). That blindness is a *feature*: a term that separated there would be manufacturing a discrimination the data does not support, which is precisely how §3.12 and §3.15 failed. **The experiment:** rerun `exp18`'s adversarial/matched $2\times2$ with `w_independence > 0`. Pre-register a *split* prediction — the adversarial arm moves off R2 **toward** R1 (not past it, as `exp18` overshot), and the symmetric-$p_B$ control does **not** move, since there the term is provably blind. Calibrate $w$ first: §3.12's lesson is that a weight never survives a change of penalty definition |
| **47** | **is Route D rigid?** | **UPDATED 2026-08-24, three steps forward on the cycle case.** **(i) §15.13's density step was never needed** — independence is a statement about *each* $g_s$, so once $u_s$ is linear the stabiliser applies **pointwise**; the `TODO(gap)` for rational $\omega_2$ dissolves, along with the conformality of $f_1$ that the density route needed. **(ii) The linearisation step closes when $f_1$ has a fixed point (§15.13a).** Apply Theorem D's own device to the Taylor jet: a degree-$p$ term is carried to $A_1^{-n}Q(A_1^{n}\cdot)$, which decays iff $(p-1)\log\rho_1+\sigma_1<0$, binding at $p=2$ — **(D-e) $\log\rho_1+\sigma_1<0$**. So all three of Route D's conditions have the *same shape*: each compares a rate to module 1's **own spread**, never a separation *between* modules, which is exactly why §3.7's two-sided obstruction misses them, and **all three are free for a conformal $f_1$**. Certified sharp (measured decay ratio matches $e^{10(\log\rho_1+\sigma_1)}$ to $<10^{-3}$ relative, five cases, both sides). **Anisotropy breaks it, not slowness** — $\operatorname{diag}(0.6,0.3)$ contracts *harder* than a $0.95$ spiral and its jet **grows** $6.2\times$ per decade; §3.14's "width, not speed" again. Price: formal $\Rightarrow$ actual needs analyticity, i.e. (FLAT-D). **This does NOT reach two oscillators** — (D-e) and $u_s(0)=0$ both need a fixed point, and task 23's *recipient* module is itself a cycle. **(iii) For two oscillators, Lemma T identifies the mechanism (§15.13b).** On a cycle the escape lives in the phase, where $\operatorname{Stab}_{\mathrm{Homeo}}(\mu)=F_\mu^{-1}SO(2)F_\mu$ is an entire circle group — far bigger than $O(2)$, which is why (D-d) looks too weak there. But independence holds at **every $t$** and $p_1^{(t)}$ is $\mu$ rotated by $t\omega_1$, so the escape must preserve two rotated copies, and **that cuts it to $\operatorname{Stab}_{O(2)}(\mu)$ exactly** — after which (D-d) finishes. It is an *identification*, not a bound: for a pure 2nd-harmonic law the one surviving non-identity element is the half turn to three decimals, matching that law's $\mathbb{Z}_2$; for a von Mises the argmin is the identity; for uniform $\mu$ everything survives, which is §15.5's Hyvärinen–Pajunen hole as the exact degenerate case. **Two time points, the same count as Lemma D's two behaviour levels, for the same reason.** **(iv) The plumbing works — Theorem D″ (§15.13c), and two oscillators CLOSES for triangular $h$.** The worry that $u$ might differ across times dissolves once the reduction is written: independence at time $t$ says $(g_\sigma)_\#p_1^{(t)}=q^{(t)}$ for $\sigma\in\operatorname{im}f_2^t$, so $u:=g_{\sigma'}^{-1}g_\sigma$ (taking $\sigma,\sigma'$ on the cycle, where $f_2$ is bijective) is **one fixed map**, defined with no reference to $t$, required to preserve $p_1^{(t)}$ for *every* $t$ — **time enters only through which law $u$ must preserve, never through which map it is.** Lemma T + (D-d) then make $g$ constant on the cycle, and **(D-b′) extends it off the cycle** via $g_s-g_*=\tilde f_1^{-n}(g_{f_2^n(s)}-g_*)f_1^{n}$, bounded by $\lVert\cdot\rVert e^{n\sigma_1}$ against approach at $\rho_2^{\perp}$. **No new hypotheses** — (D-b′) and (D-d) are Theorem D′'s, unchanged. **This kills task 23's counterexample by name**, and it survives exactly when (D-d) fails, i.e. uniform phase = §15.5's hole, pinned in `tests/test_counterexamples.py` (concentrated $\to$ conditional law moves; uniform $\to$ it does not; monotone in between, so the hypothesis is quantitative rather than a dichotomy). **Two `TODO(gap)`s remain and they differ in weight:** **Lemma T is now proved except in one case** — the CDF normal form turns membership in both stabilisers into "$\Phi:=F_\mu R_\omega F_\mu^{-1}$ commutes with $R_\alpha$"; **irrational $\alpha$ is immediate** (its centraliser is the rotations, so $\Phi$ would be a rotation and $R_\omega$ would preserve $\mu$), and the easy inclusion $\operatorname{Stab}_{O(2)}(\mu)\subseteq$ both is one line since rotations commute. `TODO(gap)` is only **rational $\alpha$**: could $\Phi$ descend to the $q$-fold quotient without $\mu$ having that symmetry? Searched over 5 laws $\times$ 2 rotations $\times$ $q\in\{2,3,4,5\}$ — every survivor had symmetry defect $0.0000$, the $\mathbb{Z}_4$ law correctly yielding both $q=2$ and $q=4$. Evidence, not proof, and pinned as a test that would *report* a counterexample. Second gap: triangularity for two cycles, which (F3) cannot supply. **(v) That gap is removed for the lattice class — Proposition L (§15.13d), and it is the direct answer to task 23.** Two cycles span a torus and a conjugacy acts on $H_1=\mathbb{Z}^2$ by $M\in GL(2,\mathbb{Z})$; independence tested against $M$ by Fourier reads $\hat\mu_1(ma+nc)\hat\mu_2(mb+nd)=\hat\mu_1(ma)\hat\mu_2(mb)\hat\mu_1(nc)\hat\mu_2(nd)$ for all $m,n$. **PROVED FOR EVERY $M$ (2026-08-24), via a master identity.** Since $\det M=\pm1$ two lattice directions are special: $(m,n)=k(d,-b)$ sends the argument pair to $(k\det M,0)$, so $\Psi=\hat\mu_1(\pm k)$, and factorising the same value through $A(kd)B(-kb)$ gives $\lvert\hat\mu_1(k)\rvert=\lvert\hat\mu_1(kad)\rvert\lvert\hat\mu_1(kbc)\rvert\lvert\hat\mu_2(kbd)\rvert^{2}$; $(m,n)=k(-c,a)$ gives the mirror. With $r_i:=\sup_{k\neq0}\lvert\hat\mu_i(k)\rvert$ the hypotheses are exactly $0<r_i<1$ ($r_i=0$ is uniform, $r_i=1$ is lattice-supported). **All entries nonzero** $\Rightarrow r_1\le r_1^2r_2^2\Rightarrow r_1=r_2=1$, excluded. **$c=0$** $\Rightarrow\lvert\hat\mu_2(kb)\rvert=1$ wherever $\hat\mu_1(k)\neq0$, so $b=0$. **$a=0$ or $d=0$** reduces to triangular by relabelling the outputs. No extra hypotheses beyond the two already stated. The index bookkeeping — where a transpose slip would be invisible — is pinned to $\le1.4\times10^{-17}$ over five matrices including a $\det=-1$ one, and the wider orbit is checked at defect $0.31$–$0.81$ against a baseline $0.004$. **The boundary is sharper than "some module is uniform":** with $\mu_1$ uniform the survivors are exactly $(1,1;0,1)$, $(1,-1;0,1)$, $(1,2;0,1)$ — those where module 1 *receives* — while $(2,1;1,1)$ is **rejected even then**, because $c\neq0$ makes the non-uniform module 2 a recipient too. The Hyvärinen–Pajunen escape needs the whole off-diagonal action pointing **into** a uniform module, which is exactly why §15.4's proviso is about the recipient. **So task 23's $GL(2,\mathbb{Z})$ ambiguity is resolved by independence alone** — no triangularity, no (F3), no non-resonance, no auxiliary variable. **(vi) And the triangularity hypothesis is then removable on the attractor — Corollary R (§15.13e).** Prop. L pins $h$'s action on $H_1$; the homotopically trivial part falls to classical rigidity. In asymptotic-phase coordinates both flows are *rigid* rotations, so writing $h_i=\theta_i+u_i(\theta)$ the conjugacy gives $u_i(\theta+\omega)-u_i(\theta)=c_i$; averaging forces $c_i=0$ (hence $\tilde\omega=\omega$), and Fourier leaves $\hat u_i(m,n)(e^{i(m\omega_1+n\omega_2)}-1)=0$. **If $\omega_1,\omega_2,2\pi$ are rationally independent every mode dies, $u_i$ is constant, and $h\vert_{T^2}$ is a signed permutation composed with a rigid translation — block-diagonal, with nothing assumed about $h$'s shape.** Three points. **(1)** This non-resonance is on *rotation numbers* and is **generic**, in sharp contrast to (B4′) on the Lyapunov spectrum, which Prop. N shows is *identically false* the moment anything rotates — so the oscillatory structure that destroys (B4′) is exactly what makes this available. **(2)** The escape is **periodicity**: at $\omega=(2\pi/3,2\pi/5)$ resonant modes give exact conjugacies (defect $1.8\times10^{-15}$ for $(3,0),(0,5),(3,5)$) against $0.23$–$0.56$ for every mode under non-resonance. **(3)** The surviving modes are graded correctly, which is the check that it is not too strong: $(m,0)$ in $u_1$ is a *within-module* reparameterisation, which §7 grants, and only modes with **both** indices nonzero break block-diagonality. **Net: for two oscillators on the attractor, independence + non-uniform non-lattice phase laws + rationally independent rotation numbers gives block-diagonality up to module permutation and within-module gauge — no triangularity, no (F3), no (B4′).** Off the attractor Theorem D″ Step 4's (D-b′) is still what extends it, and §3.8's support caveat applies. Original entry: **PARTLY PROVED — Theorem D, §15.12, at a contracting fixed point.** §15.11's reduction closes there. From independence, $u_s:=g_{s_0}^{-1}g_s$ preserves $p_1$ with $u_{s_0}=\mathrm{id}$; the conjugacy at the fixed point gives $\tilde f_1=g_{s_0}f_1g_{s_0}^{-1}$ and hence the **cocycle relation** $u_s=f_1^{-1}u_{f_2(s)}f_1$, so $u_s=f_1^{-n}u_{f_2^n(s)}f_1^{n}$ for every $n$. Since $f_2^n(s)\to s_0$ at rate $\rho_2$ and conjugation distorts by $\operatorname{cond}(f_1^n)=e^{n\sigma_1}$, $u_s=\mathrm{id}$ whenever $$\log\rho_2+\sigma_1<0,\qquad \sigma_1=\text{Lyapunov \emph{spread} of }f_1.$$ **(D-c) is NOT Lemma C's gap and that is the whole point:** it compares $f_2$'s rate to $f_1$'s *own* spread — an internal property of one module — rather than demanding a separation *between* modules, which §3.7 proves can never hold in both directions. **For a conformal $f_1$ (any `TwistBlock`, spectrum $\{\log s,\log s\}$, spread $0$) it is FREE.** So the oscillatory module that (B4′) excludes identically (Prop. N) and that (F3) cannot usefully order is exactly where Theorem D costs nothing. **Complementary to Theorem F, demonstrably:** with $f_1$ conformal at $0.8$ and $f_2$ at $0.9$, (F3) holds in order $[1,0]$ so Theorem F *permits* $h_1$ to depend on $z_2$, Lemma C's oriented gap for killing it fails, and Theorem D kills it. Condition verified **sharp**: residual $4.0\times10^{-5}$ at $\rho_2e^{\sigma_1}=0.8$ versus $1.2\times10^{32}$ at $3.6$. **Every known escape fails exactly one hypothesis** — §4.3 triangular fails (D-a) independence, $z_1\operatorname{sgn}(z_2)$ fails (D-b) continuity, the rotational escape fails (D-b) by needing a *cycle*. **The limit-cycle case is Theorem D′ (§15.13), and it is a SKETCH, weaker than Theorem D**, and it closes on Proposition S's own object: the transverse contraction replaces the fixed point ($\log\rho_2^{\perp}+\sigma_1<0$), after which the linear part is constant along dense $f_2$-orbits and independence puts it in $\operatorname{Stab}_{GL}(p_1)$ — compact by Prop. S — so **trivial $\operatorname{Stab}(p_1)$ finishes it**. `TODO(gap)` on the density step (needs $\omega_2$ irrational; the rational case needs the periodic-orbit version). **So §14 and §15 close on the SAME condition** — every module's law has no nontrivial orthogonal symmetry, i.e. whether trials are phase-aligned — which is a property of the recording, not of the spectra. Measured both ways: the rotational escape reads $0.0465$ against a $0.0401$ baseline when $p_1$ is symmetric and $\mathbf{0.8512}$ when its phase is concentrated. Original entry, **and the reduction that got here — §15.11.** Theorem F makes $h$ triangular, so write $g_s:=h_1(\cdot,s)$; independence says every $g_s$ pushes $p_1$ to the *same* law, hence $g_{s'}^{-1}g_s\in\operatorname{Stab}(p_1)$, and the conjugacy adds $g_{f_2(s)}\circ f_1=\tilde f_1\circ g_s$. **So the escape is conjecturally valued in $\operatorname{Stab}(p_1)\cap C(f_1)$** — measure-preserving *and* commuting with the module's own map. **Both known counterexamples are exactly of that form** and are now committed as tests: $h_1=z_1\operatorname{sgn}(z_2)$ ($\operatorname{Stab}=O(1)$, exact conjugacy, dCor $0.0248$ vs baseline $0.0247$, changes $50.3\%$ of samples — but discontinuous at $z_2=0$, which is *why* $d_B=1$ needs no extra hypothesis: the stabiliser is discrete) and $h_1=R(\theta(z_2))z_1$ with rotationally symmetric $p_1$ (smooth, a genuine conjugacy on a cycle per §13.2; concentrating the phase sends dCor $0.0465\to0.8512$). **THE UNIFICATION: Route B's residue (Prop. S, a coset of $\operatorname{Stab}_G(p_B)$) and Route D's hole are the SAME OBJECT**, so the two routes do not fail independently and neither is a fallback for the other — and a single checkable condition closes both: *every module's law has trivial stabiliser*. **Sharpened conjecture:** if $\operatorname{Stab}(p_i)$ is trivial for every $i$, a modular conjugacy preserving module independence is block-diagonal up to permutation. What is missing is one rigidity step — that a $p_1$-preserving family compatible with $f_1$ must be constant; plausibly the same centraliser argument as §4.5 Step 1, not yet written. Original entry: `exp21` §15 established that independence of the module marginals **rejects the §4.3 triangular conjugacy and the §7 lattice regrouping** — the two objects that block Theorem B and Theorem F respectively — while being correctly blind to §3.1's regrouping, which is (B2)'s job. It needs no non-resonance (identically false for oscillators, Prop. N), no (F3), and no auxiliary variable. **What is missing is the theorem.** Independence alone is insufficient for nonlinear ICA and the Hyvärinen–Pajunen counterexample lives inside our own setting — with *uniform* phase laws the lattice map preserves independence exactly. **Conjecture: independence of the module marginals PLUS $h$ being a modular conjugacy forces $h$ block-diagonal up to permutation.** Two things already known about its boundary: the Gaussian degeneracy is real but **excluded by (A2)** (a rotation is a modular conjugacy only at equal eigenvalues), and uniform phase laws are a genuine hole. Start from the fact that the Hyvärinen–Pajunen constructions are Darmois/CDF-based and have no reason to commute with a modular $F$ — the conjunction is where the rigidity should come from |
| **46** | **the Route B front line: does the concentration condition survive learning?** | **PARTLY ANSWERED and REDIRECTED by `exp21` (§15).** The planned fix — impose whitened circular concentration as an objective term — is **not available**: free maximisation reaches $0.9996$ by giving the block's radius a heavy tail (spread $523\times$), so the term is payable in gauge, which is §3.12 a fourth time. Caught before running it. What replaced it: the same statistic used as a **comparison across fits** rather than an objective, plus Propositions C/R′ which say the escape is strictly resultant-decreasing and that the real criterion is *independence*. Original entry: **stage 2 of the program `exp20` opened.** Stage 1 (§14) settled the analytic half: Route B kills every non-compactly-valued coupling, the residue is compact, and a whitened second-moment detector needs the pinned block to carry a $u$-dependent mean direction at circular concentration $\gtrsim0.3$. **`exp18` §13.4 is the obstacle and now also the target**: breaking $p_B$'s symmetry works in the *data* ($0.016\to0.956$) and the fitted encoder undoes it, landing at $0.270$ — which §14.4 shows is *just* under the detection threshold, so the evasion is minimal rather than dramatic. **The fix follows from Proposition S: impose the condition on the fitted representation instead of inheriting it.** Add **whitened circular concentration** of the pinned block, matched to the data's, as an objective term. Why that is not another §3.12: whitening leaves $O(d_B)$ and concentration is $O(d_B)$-invariant, so **whitened concentration is exactly $GL(d_B)$-invariant** — it cannot be paid by rescaling, shrinking or rotating the block, only by genuinely breaking the symmetry. Pre-register a *split* prediction (fitted concentration tracks the data's, **and** the adversarial arm moves off R2 *toward* R1 rather than past it — `exp18` overshot to $0.1101$ against an R1–R2 separation of only $0.0796$). **Third arm is mandatory and it must stay negative:** with $p_B$ genuinely symmetric in the data the term has to be *unsatisfiable* rather than fakeable, or it is a penalty measuring the gauge again — the failure this repo has now hit three times (§3.12, §3.15, §13.4). Reuse `exp18`'s system and seeds so the comparison is within-experiment |
| **45** | **NEW: redo M56's dynamics in a nonlinear chart** | open, and it is a *prerequisite*, not a result — see the exp19 row (g). Every dynamical reading of M56 so far (eigenvalues, (F3), contraction) was computed in **PCA space**, which `exp19` then showed is the wrong chart for the one area whose manifold is genuinely curved. Refit in an autoencoder latent space at matched bottleneck and recompute the fingerprint before relying on anything. Note this does **not** gate task 41 — `exp17`'s adversarial initialisation needs no neuron split, so M56's ~27 units are not disqualifying there |
...
- **D** (independence of module marginals) — **NEW 2026-08-25, `exp21`.** Not a
  hypothesis on the dynamics at all: the *initial-condition* distribution has
  independent module marginals. Rejects exactly the two escapes that block the
  other routes — the §4.3 triangular conjugacy and the §7 lattice regrouping —
  and is correctly blind to §3.1's, which is (B2)'s job. Needs no non-resonance,
  no (F3), no auxiliary variable. **Conjecture, not a theorem:** independence
  alone is insufficient for nonlinear ICA (Hyvärinen–Pajunen), and `exp21` part 8
  exhibits the counterexample inside our own setting — with *uniform* phase laws
  the lattice map preserves independence exactly. What is conjectured is that
  independence **plus being a modular conjugacy** is rigid.

C is a strict weakening of A; B composes with either; D is orthogonal to all
three, since it constrains the latent *distribution* rather than the map.

> **REVISED 2026-08-24 — the priority is now D, and C's own hypotheses are why.**
> The 2026-08-04 decision below picked C because it was the proved, unduplicated
> route that reached oscillations. Three measurements since have closed that
> reasoning off in the regime that matters: (F3) holds in 2/36 fits on MC_Maze
> and 0/24 on `exp14`; (B4$'$) is **identically false** wherever a rotation exists
> (Prop. N, §5.3); and Route B is structurally blind to the lattice regrouping
> (`exp18`). So all three routes are dead for two oscillatory modules, each for a
> different and now-understood reason.
>
> **Route D is the replacement and it is a different kind of hypothesis** — on
> the initial-condition *distribution*, not on the spectra. It rejects the §4.3
> triangular conjugacy and the §7 lattice regrouping, the two objects that block
> B and F, and is correctly blind to §3.1's, which is (B2)'s. Its cost is that it
> is a **conjecture**: independence alone is insufficient for nonlinear ICA, and
> `exp21` §15.9 shows an optimiser finding the hole. **Task 47 is the front
> line.** Task 37 is unaffected — per-module invariants are still the applied
> deliverable, and D is a route to the decomposition they are reported on.
...
> **Bounded from above 2026-08-17 (task 44, `exp18`).** B was the obvious
> candidate to cover the oscillatory regime, since Lemma D′ needs no spectral
> hypothesis and so applies exactly where C is dead. **It does not cover it.**
> The $GL(K,\mathbb{Z})$ regrouping acts on the invariant block by a *rotation*,
> and a rotationally symmetric $p_B$ is exactly invariant under it — so
> behaviour is blind, structurally rather than for want of a better estimator.
> Worse, the escape §4.5 excluded at a fixed point is a **genuine modular
> conjugacy** on a cycle. And breaking the symmetry in the data does not save it,
> because the encoder flattens the phase back (§13.4). **So B∘C is not the
> fallback for two oscillatory modules**, and Route B's domain is where the
> coupling is additive — which is where Lemma D always said it was. The useful
> residue is a general criterion: *the kill needs $p_B$ to have trivial symmetry
> under the group the coupling lives in*, which is also the reason additivity
> works at all.
>
> **Sharpened 2026-08-24 (`exp20`, §14), and it moves this bound UP.** That
> criterion completes into **Proposition S**: the escape is exactly a coset of
> $\operatorname{Stab}_G(p_B)$, which is always **compact**. So Route B kills
> every coupling valued in a non-compact group — translations *and* scalings
> *and* shears, certified at matched displacement — and additivity is merely the
> translation case of a much wider class. **Route B's domain is therefore larger
> than "additive coupling"**, and the sentence above understates it; what it
> cannot reach is a compact residue *conjugate to* a subgroup of $O(d_B)$, which
> is finite-dimensional and measurable with no fitting. **The bad news is one
> level down, and it is new:** the practical detector is blinder than the
> criterion, unavoidably — whitening for $GL(d_B)$-invariance (§3.12) is
> precisely what makes any second-moment detector blind to $O(d_B)$. What it can
> still see is a $u$-dependent **mean direction**, above a measured
> concentration threshold of $\approx0.3$ — which is where `exp18`'s fitted
> adversarial block ($0.270$) sits.

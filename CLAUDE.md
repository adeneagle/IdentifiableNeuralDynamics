...
| 8 | **Position against the literature** (Hyvärinen & Morioka, Hälvä & Hyvärinen, Khemakhem et al.). Be explicit about what modular *dynamics* adds beyond conditioning on a time index | drafted in `theory/literature.md` §1; needs write-up into `identifiability.md` §8. **Now sharper under §1.1**: those results all need non-stationarity or an auxiliary variable, and we have neither — autonomous + stationary is exactly the uncovered case |

...
2. `python -m pytest -q` — **expect 352 passing**, on numpy 2.4 *or* 2.5; the
...
| **41** | **NEW (ACTIVE, the front line): adversarial initialisation — turn agreement into evidence** | open. `exp16` §11.3(a) showed the task-40 protocol returns **"identified" on a system where non-identifiability is proved** (arm C, two limit cycles, agreement $0.0004$). So cross-split agreement measures **estimator reproducibility**, not identifiability, and §10.3's MC_Maze conclusion must be read that way until this closes. **The fix:** start the two fits at deliberately *different* representatives — `regrouping_counterexample`'s `system_tilde` for §3.1, `torus_regrouping_counterexample`'s lattice image for §7 — warm-start on those latents, then train normally. If the data pins the representation the fits converge back to the same invariants; if not they stay apart. That converts a necessary condition into something near sufficient, and arm C is the known-answer case that says whether it works. **Requires `encoder="mlp"` (§11.7)** — with a linear encoder the alternative is outside the fitted class and the test would measure the projection, not the data. **Also report $\min\lvert z_i\rvert$ per module** (§11.6): that, not `filtration_gap`, is what decides whether the lattice ambiguity is live |
